...
contract('DappToken', function(accounts){
  ...
  it('transfers token ownership', function(){
    return DappToken.deployed().then(function(instance){
      tokenInstance = instance;
      return tokenInstance.transfer(accounts[1], 9999999999999999999999);
      ...
      assert.equal(receipt.logs[0].args._from, accounts[0], 'logs the account the tokens are transferred form.')
      ...
      assert.equal(balance.toNumber(), 750000, 'deducts the amout from sending account')
    })
  })
  ...
  it("approves tokens for delegated transfer", function(){
    return DappToken.deployed().then(function(instance){
      ...
      assert.equal(receipt.logs[0].args._owner, accounts[0], 'logs the account the tokens are transferred form.')
  ...
  it('handles deletegated token transfers', function(){
    return DappToken.deployed().then(function(instance){
      tokenInstance   = instance;
      ...
      assert.equal(allowance.toNumber(), 0, 'deducts amount from allownce')
    })
  })
